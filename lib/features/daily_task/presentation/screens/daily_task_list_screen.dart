        ...
        if (notifier.todayTasks.isEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(IconMap.checkCircle, size: 64, color: AppColors.success),
                const SizedBox(height: AppSpacing.md),
                Text(
        ...
        children: [
          if (hasMyTasks) ...[
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    'Tugas Saya',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: theme.colors.foreground),
                  ),
                  const Spacer(),
                  Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: theme.colors.primary),
                    ),
                  ),
                ],
              ),
            ...
            ...notifier.todayTasks.map((task) => Padding(
              padding: const EdgeInsets.only(bottom: AppSpacing.sm),
              child: _DailyTaskCard(task: task, onTap: () => context.push('/daily-task/${task.id}')),
            )),
            const SizedBox(height: AppSpacing.md),
          ],
          if (hasAssignedTasks) ...[
...
class _AssignedTaskCard extends StatelessWidget {
            ...
            children: [
              Row(
                children: [
                  ...
                  const Spacer(),
                  Icon(IconMap.chevronRight, size: 20, color: theme.colors.mutedForeground),
                ],
              ),
              const SizedBox(height: AppSpacing.sm),
              Row(
                children: [
                  ...
                  if (employeeCount > 1) ...[
                    ...
                    Container(
                      ...
                      child: Text(
                        '$employeeCount orang',
                        style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: theme.colors.primary),
                      ),
                    ),
                  ],
                ],
              ...
              Text(itemName, style: TextStyle(fontSize: 14, color: theme.colors.foreground)),
              if (areaName != '-') ...[
                const SizedBox(height: AppSpacing.xs),
                Row(
                  children: [
                    ...
                    const SizedBox(width: 4),
                    Text(areaName, style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                ),
              ],
              const SizedBox(height: AppSpacing.sm),
              Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text(assignedDate ?? '-', style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  if (targetMinutes != null) ...[
                    const SizedBox(width: AppSpacing.md),
                    Icon(IconMap.timer, size: 16, color: theme.colors.mutedForeground),
                    const SizedBox(width: 4),
                    Text('$targetMinutes menit', style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                  if (durationMinutes != null) ...[
                    const SizedBox(width: AppSpacing.md),
                    Icon(IconMap.accessTime, size: 16, color: theme.colors.mutedForeground),
                    const SizedBox(width: 4),
                    Text('$durationMinutes menit', style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                ],
              ),
  ...
  Widget _buildPhotosSection(BuildContext context, FThemeData theme, List<dynamic> beforePhotos, List<dynamic> afterPhotos) {
    ...
    return Column(
      ...
      children: [
        Text(
          'FOTO',
          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: theme.colors.mutedForeground, letterSpacing: 0.5),
        ),
        const SizedBox(height: AppSpacing.xs),
        SizedBox(
          height: 80,
          child: ListView.builder(
            ...
            itemBuilder: (ctx, i) {
              ...
              return Padding(
                padding: EdgeInsets.only(right: i < allPhotos.length - 1 ? AppSpacing.xs : 0),
                child: Stack(
                  children: [
                    GestureDetector(
                        ...
                        child: ClipRRect(
                          ...
                          child: Image.network(
                            ...
                            errorBuilder: (c, e, s) => Container(
                              color: theme.colors.muted,
                              child: Icon(IconMap.brokenImage, color: theme.colors.mutedForeground),
                            ),
                          ),
                        ),
                      ),
              ...
              child: InteractiveViewer(
                ...
                child: Image.network(
                  ...
                  loadingBuilder: (context, child, loadingProgress) {
                    ...
                    return const Center(child: LoadingIndicator(color: Colors.white));
                  },
                  errorBuilder: (context, error, stackTrace) {
                    return Center(
                      child: Icon(IconMap.brokenImage, color: Colors.white54, size: 64),
                    );
                  },
                ),
              ),
    ...
    if (assignment.containsKey('employees') && assignment['employees'] != null) {
      ...
      if (employees is List) {
        return employees.map((e) {
          ...
          if (e is Map) return e['name'] as String? ?? e['employee_name'] as String? ?? '';
          return '';
  ...
  int _getEmployeeCount(Map<String, dynamic> assignment) {
    ...
    if (assignment.containsKey('employees') && assignment['employees'] != null) {
      final employees = assignment['employees'];
      if (employees is List) return employees.length;
    }

    if (assignment.containsKey('employee_names') && assignment['employee_names'] != null) {
      final names = assignment['employee_names'];
      if (names is List) return names.length;
    }

    if (assignment.containsKey('employee_name') && assignment['employee_name'] != null) return 1;
...
class _DailyTaskCard extends StatelessWidget {
  ...
  const _DailyTaskCard({required this.task, required this.onTap});
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                task.itemName,
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: theme.colors.foreground),
              ),
              ...
              if (task.areaName != null) ...[
                ...
                Row(
                  children: [
                    Icon(IconMap.locationOn, size: 16, color: theme.colors.mutedForeground),
                    const SizedBox(width: 4),
                    Text(task.areaName!, style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                ),
              ],
              if (task.targetMinutes != null) ...[
                ...
                Row(
                  children: [
                    Icon(IconMap.timer, size: 16, color: theme.colors.mutedForeground),
                    const SizedBox(width: 4),
                    Text('Target: ${task.targetMinutes} menit', style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                ),
              ],
              if (task.startAt != null && task.endAt != null) ...[
                ...
                Row(
                  children: [
                    Icon(IconMap.accessTime, size: 16, color: theme.colors.mutedForeground),
                    const SizedBox(width: 4),
                    Text('Durasi: ${task.durationMinutes ?? '-'} menit', style: TextStyle(fontSize: 13, color: theme.colors.mutedForeground)),
                  ],
                ),
              ],
